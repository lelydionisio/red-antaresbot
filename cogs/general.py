...
class RPS(Enum):
    pedra     = "\N{MOYAI}"
    papel    = "\N{PAGE FACING UP}"
    tesoura = "\N{BLACK SCISSORS}"
...
class RPSParser:
    def __init__(self, argument):
        ...
        if argument == "pedra":
            self.choice = RPS.pedra
        elif argument == "papel":
            self.choice = RPS.papel
        elif argument == "tesoura":
            self.choice = RPS.tesoura
        else:
            raise
...
class General:
    ...
    async def rps(self, ctx, your_choice : RPSParser):
        ...
        red_choice = choice((RPS.pedra, RPS.papel, RPS.tesoura))
        cond = {
                (RPS.pedra,     RPS.papel)    : False,
                (RPS.pedra,     RPS.tesoura) : True,
                (RPS.papel,    RPS.pedra)     : True,
                (RPS.papel,    RPS.tesoura) : False,
                (RPS.tesoura, RPS.pedra)     : False,
                (RPS.tesoura, RPS.papel)    : True
               }
        ...
        if outcome is True:
            await self.bot.say("{} Voce venceu {}!"
                               "".format(red_choice.value, author.mention))
        elif outcome is False:
            await self.bot.say("{} Voce perdeu {}!"
                               "".format(red_choice.value, author.mention))
        else:
            await self.bot.say("{} Empatamos :c {}!"
                               "".format(red_choice.value, author.mention))
    ...
    async def _8ball(self, *, question : str):
        ...
        if question.endswith("?") and question != "?":
            ...
            await self.bot.say("Nao tem interrogaçao poha")
    ...
    async def stopwatch(self, ctx):
        ...
        if not author.id in self.stopwatches:
            ...
            await self.bot.say(author.mention + " O relogio começou")
        else:
            tmp = abs(self.stopwatches[author.id] - int(time.perf_counter()))
            tmp = str(datetime.timedelta(seconds=tmp))
            await self.bot.say(author.mention + " Relogio pausado!! Tempo: **" + tmp + "**")
